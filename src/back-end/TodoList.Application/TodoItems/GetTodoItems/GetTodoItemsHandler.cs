...
namespace TodoList.Application.TodoItems.GetTodoItems
{
    ...
    public sealed class GetTodoItemsHandler(ILogger<GetTodoItemsHandler> logger) : IRequestHandler<GetTodoItemsCommand, Result>
    {
        private readonly ILogger<GetTodoItemsHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

        public async Task<Result> Handle(GetTodoItemsCommand request, CancellationToken cancellationToken)
        {
